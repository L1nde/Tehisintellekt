...
def createBoard():
    for i in range(boardWidth):
        for j in range(boardWidth):
            button = Button(root, text = "", font="Helvetica 16", height=3, width=6, background=tkColorButton)
...
title = Label(root, text="Kabe", fg=tkColorTitle, font="Helvetica 20 bold", background=tkColorBg).grid(row=0, column=0, padx=10, pady=10, columnspan=8)

# Laua loomine
createBoard()

# Info kuvamine
tkMessage = Label(root, text="Alustab mustade nuppudega mängija.", font="Helvetica 12 bold", fg=tkColorTitle, background=tkColorBg)
tkMessage.grid(row=9, column=0, padx=10, pady=10, columnspan=8)
